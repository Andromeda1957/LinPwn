...
void handler() {
  ...
  if (strncmp(option, "shell", 5) == 0) commands.shell();
  else if (strncmp(option, "help", 4) == 0) commands.help();
  else if (strncmp(option, "exit", 4) == 0)commands.quit();
  else if (strncmp(option, "", 1) == 0) pass();
  else
...
int main() {
  ...
  write(1, "Type help to list commands.\n\0", 28);
